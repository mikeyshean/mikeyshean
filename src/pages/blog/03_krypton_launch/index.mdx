...
On the positive side, I did take the opportunity to implement certain abstractions that I had not done the first time around, by 
creating a `GameObject` base class with all the common implementation inherited by all subclasses of 
it, such as all the Kryptonite, background objects (Stars, Shooting Stars, Planets, etc), and of course Superman himself.
The orignal game I made implemented it with ducktyping which was a fine approach, but having an explicit contract,
